...
void Spindle_Stop(void)
{
	...
	__HAL_TIM_SET_COMPARE(&SPINDLE_TIMER,SPINDLE_TIMER_CHANNEL,100);//这里应该是反了吧??
...
void Spindle_SetSpeed(uint8_t pwm_value)
{
	...
	__HAL_TIM_SET_COMPARE(&SPINDLE_TIMER,SPINDLE_TIMER_CHANNEL,100 - pwm_value);
...
//		TIM_Cmd(TIM1, DISABLE); // Disable PWM. Output voltage is zero.
		__HAL_TIM_DISABLE(&SPINDLE_TIMER);
	}
	else {
		//TIM_Cmd(TIM1, ENABLE); // Ensure PWM output is enabled.
		__HAL_TIM_ENABLE(&SPINDLE_TIMER);//add by lcj
	}
#endif
}
