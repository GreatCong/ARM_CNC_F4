...
//	TIM_Cmd(TIM9, DISABLE);
	...
	if(((settings.stepper_idle_lock_time != 0xFF) || sys_rt_exec_alarm || sys.state == STATE_SLEEP) && sys.state != STATE_HOMING) {
		...
		Delay_ms(settings.stepper_idle_lock_time);//这里会在中断中使用,可能有uknown bug
		pin_state = true; // Override. Disable steppers.
	}
	
  //add by lcj
	if(Arm_Motion_state &= ARM_STATE_LOCK_MOTOR){//处理G代码的电机解锁
	  pin_state = false; //Enable steppers.电机上锁
	}
	//add by lcj end
